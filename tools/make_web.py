#!/usr/bin/env python3
"""Genera la portada de la web de Knightmare, en los dos idiomas.
...
TXT = {
    "es": dict(
        titulo="Knightmare - desensamblado comentado",
        aviso="<b>Aqui no hay ninguna captura.</b> Todas las imagenes "
              "estan <b>dibujadas desde los bytes de la ROM</b>, ejecutando "
              "en Python los mismos descompresores, espejos y montadores de "
              "bandas que corre el Z80, y <b>cotejadas byte a byte contra la "
              "VRAM de openMSX</b>: las <b>ocho</b> fases volcadas del "
              "emulador dan <b>cero</b> diferencias en color (6.144 bytes), "
              "patrones (6.144) y las 704 casillas del mapa. El listado y las "
              "cifras se reproducen con <code>make</code>, y el reensamblado "
              "devuelve la ROM <b>byte a byte</b>.",
        claim="<b>Konami dejo dos erratas en sus tablas de coseno.</b> El "
              "cartucho lleva dos cuadrantes de coseno para sacar el seno y "
              "el coseno de un angulo, y un cuadrante de coseno solo puede "
              "bajar: estos dos suben una vez cada uno, y el valor que sube "
              "es justo el unico que se aparta de la funcion. Dentro hay "
              "ademas un truco que pide <b>izquierda y derecha a la vez</b> "
              "-imposible en un joystick- y da veintiseis vidas.",
    ...
    "en": dict(
        titulo="Knightmare - a commented disassembly",
        aviso="<b>Not one capture here.</b> Every picture is <b>drawn from "
              "the bytes of the ROM</b>, by running in Python the very same "
              "decompressors, mirrors and band builders the Z80 runs, and "
              "then <b>checked byte for byte against openMSX's VRAM</b>: the "
              "<b>eight</b> stages dumped from the emulator come out with "
              "<b>zero</b> differences in colour (6,144 bytes), patterns "
              "(6,144) and the 704 tiles of the map. The listing and the "
              "numbers are reproducible with <code>make</code>, and "
              "reassembling gives back the ROM <b>byte for byte</b>.",
        claim="<b>Konami left two typos in its cosine tables.</b> The "
              "cartridge carries two cosine quadrants to get the sine and "
              "cosine of an angle, and a cosine quadrant can only go down: "
              "these two go up once each, and the value that goes up is "
              "precisely the only one that departs from the function. There "
              "is also a cheat inside that asks for <b>left and right at "
              "once</b> -impossible on a joystick- and hands over twenty-six "
              "lives.",
        ficha=["Konami - <b>(c) Konami 1986</b>",
...
def main(argv):
    ...
    cabecera = (f'<img src="{img64(ruta_logo)}" alt="Knightmare">'
                if os.path.exists(ruta_logo)
                else "<h1>Knightmare</h1>")
